...
NSString* const SelfVoicePath=@"self";
...
- (NSString *)toString {
    ...
    if (self.selfVoicePath && [self.selfVoicePath length] > 0) {
        strIseParams = [strIseParams stringByAppendingFormat:@",%@=%@", [IFlySpeechConstant ASR_AUDIO_PATH], self.selfVoicePath];
    }
...
- (void)setDefaultParams{
    ...
    self.selfVoicePath=SelfVoicePath;
}
...
- (void)toUserDefaults{
    ...
    if(paramsDic){
        ...
        if(self.selfVoicePath){
            [paramsDic setObject:self.selfVoicePath forKey:SelfVoicePath];
        }
    }
...
+ (ISEParams *)fromUserDefaults{
    ...
    if(paramsDic){
        ...
        params.selfVoicePath=[paramsDic objectForKey:SelfVoicePath];
    }
    return params;
}
